...
void ManageUsersMenu(stUserInfo);
...
void deleteUser(stUserInfo currentUser) {
bool firstTime = 1;
int pos;
vector<stUserInfo> vAllUsers;
  loadAllUsersFromFileToVector(vAllUsers);
  while (read_check_binaryChoice("Delete Another User (y/n): ", firstTime)) {
    ...
    if ((pos = find_user(vAllUsers, readUsername("Entre Username You Want To Delete\n"))) != -1) {
      ...
      if (vAllUsers[pos].username == currentUser.username) {
        cout << "\nThis is the current user, can not be deleted\n\n";
      }
      else if (read_check_binaryChoice("Are You Sure You Want To Delete this User? (y/n): ")) {
...
void ManageUsersMenu(stUserInfo currentUser) {
short choice;
  while (true) {
